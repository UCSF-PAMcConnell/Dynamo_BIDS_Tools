...
def update_json_file(json_filepath):
    ...
    adds the 'TaskName' field with the value 'learn'.
...
if __name__ == "__main__":
    ...
    for suffix in ['', '_seqexec_PRE']:
        run_seqexec_PRE = 0; 
        dicom_dir = os.path.join(base_dicom_dir, f'sms3_TASK_{suffix}')
    for suffix in ['', '_seqexec_POST']:
        run_seqexec_POST = 8; 
        dicom_dir = os.path.join(base_dicom_dir, f'sms3_TASK_{suffix}')
    for run in range(1,6):
            dicom_dir = os.path.join(base_dicom_dir, f'sms3_TASK_{run}')
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                for old_file in os.listdir(tmpdirname):
                    ...
                    new_file = f"{subject_id}_{session_id}_task-learn_run-{run:02d}_bold"
