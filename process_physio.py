...
def save_segments_to_tsv_and_json(segments, bids_root_dir, subject_id, session_id, original_labels, run_index, sampling_rate):
    ...
    metadata = {
        ...
        "StartTime": segments[0]['start_index'] / sampling_rate,
        "Columns": ["cardiac", "respiratory", "eda"],
        "Manufacturer": "Biopac",
        "cardiac": {
        ...
        "eda": {
            "Description": "continuous measurements by foot electrodermal",
            "Units": "microsiemens"
        }
    }
...
def main(physio_root_dir, bids_root_dir):
    ...
    while True:
        # Debugging: Print the index from where the next segment search will start
        ...
        print(f"Saved JSON metadata for run {run_index} to {output_dir} with filename {json_file_name}")

        # If the JSON file does not exist, break out of the loop
        if not os.path.exists(json_file_path):
            print(f"No JSON file found for run {str(run_idx).zfill(2)}. Ending search for runs.")
            break

        # Debugging: Print the current run being processed
        print(f"Processing subject: sub-{subject_id}, ses-{session_id}, run-{str(run_idx).zfill(2)}")
        ...
        # Check if the segments for the current run are valid and non-empty
        if runs:
            current_run_segment = runs[0]
            all_runs.append(current_run_segment)
            save_segments_to_tsv_and_json([current_run_segment], bids_root_dir, subject_id, session_id, labels, run_idx)
    ...
    # Plot the full data with all run segments highlighted after processing all runs
