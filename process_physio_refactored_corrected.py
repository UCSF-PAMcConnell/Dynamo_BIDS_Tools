...
def segment_data_into_runs(data, trigger_starts, tr, sampling_rate, num_volumes_per_run, start_from_index=0):
    ...
    while i < len(trigger_starts):
        # Find the next sequence of triggers that could represent a run
        run_end_idx = i + num_volumes_per_run
        if run_end_idx > len(trigger_starts):
            break  # Not enough triggers left for a full run

        # Calculate the expected interval between the first and last trigger of the run
        expected_interval = samples_per_volume * (num_volumes_per_run - 1)
        actual_interval = trigger_starts[run_end_idx - 1] - trigger_starts[i]

        # Check if the actual interval is close enough to the expected interval
        if abs(actual_interval - expected_interval) <= expected_interval * 0.1:  # Allowing 10% variability
            ...
            i = run_end_idx  # Skip to the end of this run for the next iteration
        else:
            i += 1  # Move to the next trigger point

    if not runs:
        logging.info(f"No valid segments found after index {start_from_index}.")
    return runs, i
...
def save_run_segments(segments, bids_root_dir, subject_id, session_id, filtered_labels, run_idx, sampling_rate):
    ...
    new_labels = save_segments_to_tsv_and_json(segments, bids_root_dir, subject_id, session_id, filtered_labels)
    ...
    generate_and_save_metadata(physio_json_metadata, bids_root_dir, subject_id, session_id, run_idx)
...
def plot_full_data_with_segments(data, runs, sampling_rate, original_labels, output_fig_path):
    ...
    Args:
    - data: The physiological data as a numpy array.
    - runs: List of dictionaries with 'start_index' and 'data' for each run.
    - sampling_rate: The sampling rate of the data.
    - original_labels: The labels for each channel in the data.
    - output_fig_path: The file path to save the output plot.
    """

    # Convert original_labels to a list if it's a numpy array
...
def process_and_save_run(data, trigger_starts, bids_root_dir, subject_id, session_id, run_idx, sampling_rate, filtered_labels, start_from_trigger_index):
    # Load JSON metadata
    json_file_name = f"sub-{subject_id}_ses-{session_id}_task-rest_run-{str(run_idx).zfill(2)}_bold.json"
    json_metadata = load_json_metadata(os.path.join(bids_root_dir, f"sub-{subject_id}", f"ses-{session_id}", 'func', json_file_name))
    runs, next_start_index = segment_data_into_runs(data, trigger_starts, json_metadata['RepetitionTime'], sampling_rate, json_metadata['NumVolumes'], start_from_trigger_index)

    if runs:
        try:
            save_run_segments(runs, bids_root_dir, subject_id, session_id, filtered_labels, run_idx, sampling_rate)
...
def main(physio_root_dir, bids_root_dir, filtered_labels):
    ...
    while True:
        ...
        next_start_index = process_and_save_run(data, trigger_starts, bids_root_dir, subject_id, session_id, run_idx, sampling_rate, filtered_labels, start_from_trigger_index)
        if next_start_index is None:
            break # No valid segments found, stop processing
        else:
            start_from_trigger_index = next_start_index

        # Increment run index for the next iteration
            run_idx += 1
